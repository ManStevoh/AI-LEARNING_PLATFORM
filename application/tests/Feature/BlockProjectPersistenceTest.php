...
class BlockProjectPersistenceTest extends TestCase
{
    ...
    public function test_learner_can_save_project_envelope_with_checkpoints(): void
    {
        $this->seed(CurriculumFoundationSeeder::class);
        ...
        $this->actingAs($user)->postJson("/learner/learn/{$lessonSlug}/project", [
            'workspace' => $workspace,
            'generated_code' => 'await runtime.markCheckpoint("intro-blocks");',
        ])->assertOk()->assertJsonPath('saved_project.schema_version', '2.4');

        $this->withoutVite()->actingAs($user)->get("/learner/learn/{$lessonSlug}")
            ->assertOk()
            ->assertInertia(fn (Assert $page) => $page
                ->has('savedProject', fn (Assert $project) => $project
                    ->where('schema_version', '2.4')
                    ->where('workspace.checkpoints.0', 'intro-blocks')
                    ->where('workspace.checkpoints.1', 'first-animation')
                    ->etc()
                )
            );
    }
